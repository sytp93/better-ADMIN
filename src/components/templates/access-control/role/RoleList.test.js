...
//     const permissionButton = screen.getByRole('button', {name: 'right 펼쳐 보기'});
...
describe('역할 이름 검색 기능 테스트', () => {
  test('역할 이름으로 검색 하였을 때, 검색 된 데이터가 조회된다.', async () => {
    when(AccessControlService.getRoles).calledWith({
      page: 1,
      pageSize: 10,
    }).mockResolvedValue({
    ...
    await waitFor(() => {
      expect(AccessControlService.getRoles).toHaveBeenCalledWith({
        page: 1,
        pageSize: 10,
        name: '시스템',
      });
    ...
    // expect(bodyRows[0].querySelectorAll('td')[0]).toHaveTextContent('사전정의');
    // expect(bodyRows[0].querySelectorAll('td')[1]).toHaveTextContent('시스템 관리자');
    // expect(bodyRows[0].querySelectorAll('td')[2]).toHaveTextContent('MANAGE_SYSTEM_SETTING');
    // expect(bodyRows[0].querySelectorAll('td')[2]).toHaveTextContent('VIEW_MONITORING');
    // expect(bodyRows[0].querySelectorAll('td')[3]).toHaveTextContent('설명');
    // expect(bodyRows[0].querySelectorAll('td')[4]).toHaveTextContent('');

  });
  test('검색 결과에서 초기화 버튼을 누르면 검색 조건을 초기화하고 초기화된 검색 조건에 따른 결과를 보여 준다.', async () => {
    // given
    // 검색 결과에 대한 Mock
    when(AccessControlService.getRoles).calledWith({
      page: 1,
    ...
    const pagingRouter = createMemoryRouter(routes, {
      initialEntries: ['/', '/access-control/roles?page=1&pageSize=10&name=시스템'],
      initialIndex: 1,
    });

    render(<RouterProvider router={pagingRouter} />);
    await waitFor(() => screen.getByTestId('role-search'));
    ...
    // then
    const roleData = screen.getByTestId('role-table');
    const bodyRows = roleData.querySelectorAll('tbody > tr');

    fireEvent.click(permissionButton);
    //expect(screen.getByTestId('role-table').querySelectorAll('tbody > tr')).toBe();
    expect(bodyRows[0].querySelectorAll('td')[0]).toHaveTextContent('사전정의');
    expect(bodyRows[0].querySelectorAll('td')[1]).toHaveTextContent('시스템 관리자');
    expect(bodyRows[0].querySelectorAll('td')[2]).toHaveTextContent('MANAGE_SYSTEM_SETTING');
    expect(bodyRows[0].querySelectorAll('td')[2]).toHaveTextContent('VIEW_MONITORING');
